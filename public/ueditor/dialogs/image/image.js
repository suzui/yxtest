...
(function () {
    ...
    function insertImage(imgObjs) {
    	var imageObjs = utils.isArray(imgObjs) ? imgObjs : [imgObjs];
    	for(var i=0;i<imageObjs.length;i++){
    		var imageObj=imageObjs[i];
    		var tmpUrl=imageObj.src;
    		imgReady(imageObj.src,function(){
    			imageObj.src=this.src;
    			if(this.width>400){
     	    		var percent=imageObj.width/imageObj.height;
     				imageObj.width=400;
     				imageObj.height=imageObj.width/percent;
     			} 
    		    editor.fireEvent('beforeInsertImage', imageObj);
...
var imgReady = (function () {
	...
	return function (url, ready, load, error) {
		...
		if (!onready.end) {
			...
			if (intervalId === null) intervalId = setInterval(tick, 0);
		};
	};
})();
